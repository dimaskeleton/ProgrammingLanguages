...
(define the-grammar
  '((program (expression) a-program)
    ...
    (expression ("[" expression "where" (arbno identifier "is" expression) "]") where-exp)
    ))
...
(define (value-of exp env)
  (cases expression exp
    ...
    (where-exp (body vars exps)
               (let [(vals (map (lambda (e) (value-of e env)) exps))] ; Evaluates the 'exps' in the current environment, storing the result in 'vals' 
                 (value-of body ; Evaluates the 'body' of the expression in an extended environment
                           (foldr (lambda (var val acc) ; Recursivley extends the environment
                                    (extend-env var val acc)) ; Extends the environment by binding 'var' to 'val' for each expression pair
                                  env
                                  vars
                                  vals)))) 
    ))
...
; Tests for where-expressions 

(check-equal? (eval "[-(x, y)
                     where x is 10
                           y is 4]")
              (num-val 6))

(check-equal? (eval "let a = 100
                         b = 30
                     in [-(a, -(b, c))
                     where a is 200
                           c is 5]")
